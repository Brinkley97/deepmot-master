...
def missedMatchErrorV3_tracktor(prev_id, gt_ids, hypo_ids, tracks, colsoftmaxed, toUpdate):
    """
    ID switching error
    :param input: MunkresNet Output Variable Matrix of shape [batch, h, w]
    ...
    # updated_gt_ids: missed objects gt_id == -1
    updated_gt_ids = updateCurrentListV3(colsoftmaxed, gt_ids)
    id_switching = torch.zeros(1).float().cuda()
    # remove the row of missed class
    softmaxed = colsoftmaxed[:, :-1, :]
    ...
    toputOne_h = []
    toputOne_w = []
    # gt_ids may have some new object ids
    # to record hypo ids needed to switch target images ex. [1,2, 3,4, 5,6....]
    toswitch = []

    for w in range(len(updated_gt_ids)):
        _, idx = torch.max(softmaxed[0, :, w], dim=0)
        # lost object or new (not present before) object or both
        if updated_gt_ids[w] == -1 or (gt_ids[w] not in prev_id.keys()):
            # print("gt id is lost:", gt_ids[w])
            if gt_ids[w] in prev_id.keys():  # not new object (already in historic tracks) but lost

                if prev_id[gt_ids[w]] in hypo_ids:  # in historic tracks and still in current tracks
                    # sum up all the elements in this column except the prev_id[gt_ids[w]].
                    tmp = list(range(len(hypo_ids)))
                    tmp.pop(hypo_ids.index(prev_id[gt_ids[w]]))
                    id_switching = id_switching + torch.sum(softmaxed[0, tmp, w])

                else:  # in historic tracks but not in current tracks
                    # it is possible since prev_id has all the associations of
                    # ALL (not only previous frame) previous frames
                    id_switching = id_switching + torch.sum(softmaxed[0, :, w])
        ...
        # not missing and id in historic prev id
        # if object w is not assigned to an target int(hypo_ids[idx]) the same as previous target
        # prev_id[int(updated_gt_ids[w])]
        elif prev_id[updated_gt_ids[w]] != hypo_ids[int(idx)]:
            # real idSW case
            if prev_id[updated_gt_ids[w]] in hypo_ids:
                # if the id still in current track
                tmp_idx = hypo_ids.index(prev_id[updated_gt_ids[w]])  # index of previous hypo id to this gt_id
                tmp = list(range(len(hypo_ids)))
                tmp.pop(tmp_idx)
                id_switching = id_switching + torch.sum(softmaxed[0, tmp, w])
                ...
                # switch target templates
                if toUpdate and prev_id[updated_gt_ids[w]] not in toswitch:  # if pair not yet switched
                    # add to switched list, prevent switch two times
                    toswitch.append(prev_id[updated_gt_ids[w]])
                    toswitch.append(hypo_ids[int(idx)])

                    #
                    old_track_id_idx_curr = hypo_ids.index(prev_id[updated_gt_ids[w]])
                    tmp_track_a = tracks[old_track_id_idx_curr]

                    new_track_id_idx_curr = int(idx)
                    tmp_track_b = tracks[new_track_id_idx_curr]

                    #switch their positions
                    cache = tmp_track_a.id
                    tmp_track_a.id = tmp_track_b.id
                    tmp_track_b.id = cache

                    tracks[old_track_id_idx_curr] = tmp_track_a
                    tracks[new_track_id_idx_curr] = tmp_track_b

            else:
                # if the old id still not in current track
                id_switching = id_switching + torch.sum(softmaxed[0, :, w])

                # todo should not minimize distance right ?

                toputOne_w.append(w)
                toputOne_h.append(int(idx))

            # update prev id to current assignment
            pre_id[updated_gt_ids[w]] = hypo_ids[int(idx)] + 0
    ...
    mask_for_motp = createMatrix(toputOne_h, toputOne_w, softmaxed.size(1), softmaxed.size(2))

    return [id_switching, mask_for_motp, pre_id, tracks]


def missedMatchErrorV3_tracktor_reid(prev_id, gt_ids, hypo_ids, tracks, colsoftmaxed, colsoftmaxed_app, toUpdate):
    """
    ID switching error
    :param input: MunkresNet Output Variable Matrix of shape [batch, h, w]
    :param prev_id: dictionary of [gt_object_id (INT): hypothesis_id(INT)] of previous frame
    ...
    updated_gt_ids = updateCurrentListV3(colsoftmaxed, gt_ids)
    id_switching = torch.zeros(1).float().cuda()
    id_switching_app = torch.zeros(1).float().cuda()
    # remove the row of missed class
    softmaxed = colsoftmaxed[:, :-1, :]

    softmaxed_app = colsoftmaxed_app[:, :-1, :]
    ...
    for w in range(len(updated_gt_ids)):
        ...
        if updated_gt_ids[w] == -1 or (gt_ids[w] not in prev_id.keys()):
            ...
            if gt_ids[w] in prev_id.keys():  # not new object (already in historic tracks) but lost
                ...
                if prev_id[gt_ids[w]] in hypo_ids:  # in historic tracks and still in current tracks
                    ...
                    id_switching_app = id_switching_app + torch.sum(softmaxed_app[0, tmp, w])
                    ...
                    id_switching_app = id_switching_app + torch.sum(softmaxed_app[0, :, w])
        ...
        # prev_id[int(updated_gt_ids[w])]
            ...
            id_switching_app = torch.sum(softmaxed_app[0, tmp, w]) + id_switching_app
            toputOne_w.append(w)
            toputOne_h.append(int(idx))

    mask_for_motp = createMatrix(toputOne_h, toputOne_w, softmaxed.size(1), softmaxed.size(2))

    return [id_switching, id_switching_app, mask_for_motp, pre_id, tracks]
...
def deepMOTPperFrame(D, rowsoftmaxed):
    ...
    matched_objects = torch.sum(rowsoftmaxed.view(1, -1), dim=1)
