...
def getWarpMatrix(im1, im2):
    ...
    criteria = (
        cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 100, 0.00001)
    cc, warp_matrix = cv2.findTransformECC(im2Gray_ref, im1Gray, warp_matrix, warp_mode,criteria=criteria)
...
def IOUmask_fast_tracktor(boxA, boxesB):
    """
    ...
    # compute the area of intersection rectangle
    interArea = torch.max(torch.zeros(1).cuda(), xB - xA + 1) * torch.max(torch.zeros(1).cuda(), yB - yA + 1)
...
def calculate_dist_fastV4_torch_tracktor(bbox_det, bbox_gt, im_h, im_w):
    """
    ...
    c_gt_x, c_gt_y = 0.5 * (bbox_gt[:, 0] + bbox_gt[:, 2]), 0.5 * (bbox_gt[:, 1] + bbox_gt[:, 3])
...
def make_single_matrix_torchV2_fast_tracktor(gt_bboxes, track_bboxes, img_h, img_w):
    """
    ...
    tmp = []
    for i in range(N):
        iou = IOUmask_fast_tracktor(track_bboxes[i], gt_bboxes[:, -4:])
        l2_distance = calculate_dist_fastV4_torch_tracktor(track_bboxes[i], gt_bboxes[:, -4:], img_h, img_w)
        tmp.append(0.5*(l2_distance + (1.0 - iou)))
    dist_mat = torch.cat(tmp, dim=0)

    return dist_mat.unsqueeze(0)
