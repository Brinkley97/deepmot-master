...
class ResNet(models.ResNet):
    ...
    def build_crops(self, image, rois):
        ...
        for cou, r in enumerate(rois):
            ...
            x1 = int(r[2])
            y1 = int(r[3])
            # clip roi within the image
            x0 = np.clip(x0, 0, image.shape[3]-1)
            x1 = np.clip(x1, 0, image.shape[3]-1)
            y0 = np.clip(y0, 0, image.shape[2]-1)
            y1 = np.clip(y1, 0, image.shape[2]-1)
            # print(image.shape)
