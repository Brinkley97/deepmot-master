...
class FPN(FPNResNet):
    ...
    def extract_features_grad(self, rois):
        rois = rois.cuda()
        padding = torch.zeros(rois.size(0), 1).cuda()
        rois_padd = torch.cat((padding, rois), 1)
        # rois_padd = Variable(rois_padd)
    ...
    def test_rois(self, rois):
        ...
        if bbox_pred.shape[0] != rois.shape[0]:
            print()
        ...
        cls_score = cls_score.squeeze(dim=0)
        cls_prob = cls_prob.squeeze(dim=0)
        if box_deltas.shape[0] != rois.shape[0]:
            print()
