...
def shift_box(bbox, img_h, img_w):
    ...
    # half w or half h * [0.05, 0.1]
    shift_w = (0.05*np.random.rand() + 0.05) * 0.5 * w
    shift_h = (0.05*np.random.rand() + 0.05) * 0.5 * h

    c_x += shift_w * np.random.choice([-1.0, 1.0])
    c_y += shift_h * np.random.choice([-1.0, 1.0])

    new_box = [c_x - 0.5 * w, c_y - 0.5 * h, c_x + 0.5 * w, c_y + 0.5 * h]
...
def scale_box(bbox, img_h, img_w):
    ...
    # [0.8, 1.5]
    new_h = (0.7*np.random.rand() + 0.8) * h
    new_w = (0.7*np.random.rand() + 0.8) * w

    if new_h <= 10 or new_w <= 10:
        return bbox

    return np.asarray([c_x-0.5*new_w, c_y-0.5*new_h, c_x+0.5*new_w, c_y+0.5*new_h])
